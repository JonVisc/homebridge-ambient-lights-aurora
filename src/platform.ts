...
export class AmbientLightAuroraPlatform implements DynamicPlatformPlugin {
    ...
    discoverDevices() {
        ...
        const pis = [{
            ...
            name: this.config.piName,
        }];

        for (const pi of pis) {
            const uuid = this.api.hap.uuid.generate(pi.name);
            const existingAccessory = this.accessories.find(accessory => accessory.UUID === uuid);
            if (existingAccessory) {
                // the accessory already exists
                this.log.info('Restoring existing accessory from cache:', pi.name);
                existingAccessory.context.device = pi;
                new AmbientLightAuroraPlatformAccessory(this, existingAccessory);
                this.api.updatePlatformAccessories([existingAccessory]);
            } else {
                this.log.info('Adding new accessory:', pi.name);
                const accessory = new this.api.platformAccessory(pi.name, uuid);
                accessory.context.device = pi;
